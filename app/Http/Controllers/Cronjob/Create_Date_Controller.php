        ...
        foreach ($get_emp as $key => $row) {
            ...
            if ($work_date_count == '0') {
                ...
                if (in_array(Carbon::createFromFormat('Y-m-d', $date)->isoFormat('d'), explode(',', $get_team->team_day_off))) {
                    ...
                    $check_holiday = holiday::where('emp_code', $row->emp_code)->where('emp_team', $row->emp_team)
                                            ->whereDate('holiday_date_start', '<=', Carbon::createFromFormat('Y-m-d', $date)->format('Y-m-d'))->whereDate('holiday_date_end', '>=', Carbon::createFromFormat('Y-m-d', $date)->format('Y-m-d'))
                                            ->count();
                    // ถ้ามี ลาล่วงหน้า                  
                    if($check_holiday == '1') {
                        // ข้อมูล Holiday
